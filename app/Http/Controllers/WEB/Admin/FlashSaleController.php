...
class FlashSaleController extends Controller
{
    ...
    public function update(Request $request){
         if(!auth()->user()->can('FlashSale.update')){
            abort(403, 'Unauthorized action.');
        } 
        $rules = [
            'title'=>'required',
    ...
    public function flash_sale_product(){
        if(!auth()->user()->can('FlashSale.flash_sale_product')){
            abort(403, 'Unauthorized action.');
        } 
    ...
    public function store(Request $request){
         if(!auth()->user()->can('FlashSale.store')){
            abort(403, 'Unauthorized action.');
        } 
    ...
    {
        if(!auth()->user()->can('FlashSale.destroy')){
            abort(403, 'Unauthorized action.');
        } 
        
        $flash_sale_product = FlashSaleProduct::with('product')->where('id',$id)->first();
        
        Product::where('id', $flash_sale_product->product->id)->update([
            'offer_price'  => 0    
        ]);
